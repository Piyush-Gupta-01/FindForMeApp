...
const configureClient = async () => {
    ...
    auth0Client = await auth0.createAuth0Client({
        ...
        authorizationParams: {
            redirect_uri: config.redirectUri, // Use the redirectUri from the config
        },
    });
};
...
const login = async () => {
    try {
        await auth0Client.loginWithRedirect({
            authorizationParams: {
                redirect_uri: "https://piyush-gupta-01.github.io/FindForMeApp/index.html", // Use the provided redirectUri
            },
        });
    } catch (error) {
